...
createNewPath([options.detPath 'tmps\' ]);
...
while(~feof(fidSeq))
    ...
    tmpPath = [options.detPath 'tmps\' seqName '_Det_' detectorName '.txt'];
...
while(~feof(fidSeq))
    ...
    oldPath = [options.detPath 'tmps\' seqName '_Det_' detectorName '.txt'];
