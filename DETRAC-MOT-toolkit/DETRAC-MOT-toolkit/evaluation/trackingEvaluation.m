...
if(strcmp(options.motmetric, 'DETRAC-MOT'))
    scoreSet = 0:options.trackingThreStep:1; % threshold of detection scores
else
    scoreSet = options.detectionThreshold; % threshold of detection scores
end
    ...
    for idSeq = 1:length(sequences)
       ...
       %% evaluate the tracker under each detections configuration
        for idThre = 1:length(scoreSet)
           %% parse detection
            ...
            if(exist([resultSavePath '_LX.txt'], 'file') && exist([resultSavePath '_LY.txt'], 'file')&&...
                ...
                metrics = printSeqEvaluation(seqName, gtInfo, stateInfo, folderName{idThre}); % print evaluation for each sequence
        ...
        if(options.printEvaluationForEachSeq)
            dlmwrite([res_path detectorName '/' seqName '_mot_result.txt'], allMetrics);
        end
